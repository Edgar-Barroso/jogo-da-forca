...
export function Guy({ score, ...props }) {
  return (<BodyPart {...props} name="upperBody">
    {score < 5 && <BodyPart {...props} name="head" config={joints['neckJoint']} render={<Face />} />}
    {score < 4 && <BodyPart {...props} name="upperLeftArm" config={joints['leftShoulder']}>
    ...
    {score < 5 && <BodyPart {...props} name="pelvis" config={joints['spineJoint']}>
      {score < 2 && <BodyPart {...props} name="upperLeftLeg" config={joints['leftHipJoint']}>
        <BodyPart {...props} name="lowerLeftLeg" config={joints['leftKneeJoint']} />
      </BodyPart>}
      {score < 1 && <BodyPart {...props} name="upperRightLeg" config={joints['rightHipJoint']}>
